...
func _physics_process(delta: float) -> void:
	...
	var vertical_velocity: float = self.linear_velocity.y
	buoyant_force -= vertical_velocity * damping * delta
	buoyant_force = clamp(buoyant_force, -max_velocity, max_velocity)

	var damped_velocity: Vector3 = self.linear_velocity.lerp(Vector3.ZERO, damping * delta)
	self.angular_velocity = damped_velocity
