...
- features:
	- add a way for the campaign to load the menu background level
- improvements:
	- cars need to emit sounds more often (for the attention system)
	- add icons to the interactive component of items, use those in related messages
	- add an out-of-stamina message to the history with an appropriate icon
	- find a better street light mesh
- bugs:
	- fix zombie equipment errors ("Unable to equip weapon: ZombieMelee")
	- fix leaking relationships ("More than one entity wielding...")
	- looking down lets you equip the shotgun by pressing interact (inventory items still have collision)
	- remove the extra markers from the ChainPath3D node
- docs:
	- add gunshot texture to credits
	- add fire particle texture to credits
- tools:
	- add an editor tool for random Y rotation
